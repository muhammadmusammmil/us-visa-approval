...
from src.us_visa_approval.entity.config_entity import ModelTrainingConfig
from src.us_visa_approval.entity.artifact_entity import DataTransformationArtifact, ModelTrainerArtifact, ClassificationMetricArtifact
...
class ModelTrainer:
    def __init__(self, data_transformation_artifact: DataTransformationArtifact, model_trainer_config: ModelTrainingConfig):
        ...
        self.data_transformation_artfact = data_transformation_artifact
    ...
    def initate_model_trainer(self, ) -> ModelTrainerArtifact:
        ...
        try:
            train_arr = load_numpy_array_data(filepath=self.data_transformation_artfact.transformed_train_file_path)
            test_arr = load_numpy_array_data(filepath=self.data_transformation_artfact.transfromed_test_file_path)

            best_model_detail, metric_artifact = self.get_model_object_and_report(train=train_arr, test=test_arr)

            preprocessing_obj = load_object(filepath=self.data_transformation_artfact.transformed_object_file_path)
